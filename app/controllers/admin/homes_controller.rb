class Admin::HomesController < Admin::ApplicationController
  ...
  def search_for(option)
    # 全て表示の場合
    if option == "all"
      Problem.page(params[:page])
    # 回答受付中の場合
    elsif option == "unsolved"
      @members = Member.where(is_deleted: false)
      Problem.where(best_answer_id: nil, member_id: @members.ids).page(params[:page])
    # 解決済みの場合
    elsif option == "solved"
      @members = Member.all
      Problem.where.not(best_answer_id: nil).where(member_id: @members.ids).page(params[:page])
    # 回答受付終了の場合
